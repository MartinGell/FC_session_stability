...
#datasetdir = 's3://3T7T/xcpd'

# name the directory to save data to
dataset = 'adultcontrols' #'subpop'
...
sublist = wd / 'code/sublist/adultcontrols_subject_session_run.csv'
...
for s_i, ses_dict in sub_ses_run_map.items():
    ...
    for new_ses, runs in ses_dict.items():
        ...
        for run_i in runs:
            ...
            s3_loc =  f'{datasetdir}/sub-{sub_i}_ses-combined/sub-{sub_i}/{ses_i}'
